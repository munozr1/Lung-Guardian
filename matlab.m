...
title('counting coughs');
xlabel('Time (s)');
ylabel('|Acceleration|, No Gravity');
legend({'|Acceleration|, No Gravity', 'coughs'})
hold off;


%transform data for better storing in db
% Assuming you already have the 'pks' and 'locs' variables from the previous code

% Calculate the total duration of data
totalDuration = t(end) - t(1);

% Define the time interval for each sample
sampleInterval = totalDuration / length(magNoG);

% Create a time vector for each sample point
timeVector = t(1):sampleInterval:t(end);

% Initialize an array to represent the peaks as '1's and non-peaks as '0's
peakArray = zeros(size(timeVector));

% Set the elements corresponding to peak locations as '1's
peakArray(round(locs / sampleInterval)) = 1;

% Convert the peakArray into a string
peakString = num2str(peakArray);

% Display the resulting peak string
disp(peakString);
